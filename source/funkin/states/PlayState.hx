...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ENTER && startedCountdown && canPause)
		{
			...
			openSubState(new funkin.substates.PauseSubState(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
		}
		...
		if (FlxG.keys.justPressed.EIGHT)
		{
			...
			//openSubState(new funkin.substates.ResultsState());
		...
		if (health <= 0)
		{
			...
			openSubState(new funkin.substates.GameOverSubstate(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
		}
